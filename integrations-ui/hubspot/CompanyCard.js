    ...
    return (<Card>
      ...
      <div className="px-3 py-3 flex flex-col gap-3">
        <div className="flex items-center gap-3">
          {favicon && (<img src={favicon} alt="" width={32} height={32} className="rounded-md bg-zinc-100 dark:bg-zinc-800 flex-shrink-0"/>)}
          <div className="min-w-0 flex-1">
            <div className="text-zinc-900 dark:text-zinc-100 font-medium truncate">{p.name}</div>
            {p.description && (<div className="text-zinc-500 text-xs line-clamp-2">{p.description}</div>)}
          </div>
        </div>
        ...
        <DataList items={[
            ...
            ...(p.open_deal_count !== undefined && p.open_deal_count > 0 ? [{
                  ...
                  <span className="text-zinc-900 dark:text-zinc-100 font-medium">{p.open_deal_count}</span>
                  {p.open_deal_total !== undefined && (<span className="text-zinc-500">· {formatUSD(p.open_deal_total)}</span>)}
                </span>),
                }] : []),
        ]}/>
      </div>
