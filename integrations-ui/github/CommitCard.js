...
const previewSample = {
    ...
    body: "The retry policy was duplicated across the four webhook receivers. " +
        "Extract into shared/retry.ts and apply consistently across stripe, " +
        "hubspot, github, linear handlers.",
...
export default function CommitCard(props) {
    ...
    if (props.compact) {
        ...
        <span className="font-mono text-xs text-zinc-500 flex-shrink-0">{sha}</span>
        <span className="text-sm text-zinc-900 dark:text-zinc-100 truncate">{p.subject}</span>
        <span className="ml-auto text-xs text-zinc-500 flex-shrink-0">{timeAgo(p.committed_at)}</span>
      </Card>);
    }
    return (<Card>
      ...
      <div className="px-4 py-3 flex flex-col gap-3">
        ...
        <div className="flex items-center gap-2 text-xs">
          ...
          <span className="text-zinc-900 dark:text-zinc-100 font-medium">{p.author.login}</span>
          <span className="text-zinc-500">committed {timeAgo(p.committed_at)}</span>
        ...
        {p.body && (<p className="text-sm text-zinc-700 dark:text-zinc-300 whitespace-pre-wrap break-words leading-relaxed line-clamp-3">
            {p.body}
          </p>)}
        ...
        {p.files.length > 0 && (<DataList items={[
                {
                    ...
                    {p.files.slice(0, 5).map((f) => (<li key={f} className="text-zinc-700 dark:text-zinc-300 truncate">{f}</li>))}
                    {p.files.length > 5 && (<li className="text-zinc-500">+{p.files.length - 5} more</li>)}
                  </ul>),
                },
            ]}/>)}

        {p.verified && (<div className="flex items-center gap-1.5 text-[11px] text-zinc-500">
