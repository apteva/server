...
// providerExclusion records a provider that model policy removed from the
// eligible pool, and why. Dropping silently used to make substitutions
// undiagnosable: a caller that asked for one provider and got another had
// no way to tell whether policy filtered it or it was never in scope.
...
func eligibleProviderPool(pool []ProviderInfo) []ProviderInfo {
	result, _ := eligibleProviderPoolWithExclusions(pool)
	return result
}

// eligibleProviderPoolWithExclusions is eligibleProviderPool plus the reason
// each dropped provider was dropped, so a caller resolving an explicit pin
// can explain the miss instead of silently substituting.
func eligibleProviderPoolWithExclusions(pool []ProviderInfo) ([]ProviderInfo, []providerExclusion) {
	result := make([]ProviderInfo, 0, len(pool))
	var excluded []providerExclusion
	for _, info := range pool {
		if info.ModelPolicy != nil {
			if reason := providerIneligibleReason(info); reason != "" {
				excluded = append(excluded, providerExclusion{
					Provider: providerKeyFromName(info.Type),
					Reason:   reason,
				})
				continue
			}
...
// providerIneligibleReason names the specific model-policy failure that makes
// info unusable for agent reasoning, or "" when info is eligible. The checks
// mirror the original single boolean expression, in the same order.
func providerIneligibleReason(info ProviderInfo) string {
	switch {
	case info.ModelSelectionError:
		return "model selection failed"
	case info.ModelLarge == "":
		return "no large model resolved"
	case info.ModelMedium == "":
		return "no medium model resolved"
	case info.ModelSmall == "":
		return "no small model resolved"
	case !info.allowsModel(info.ModelLarge):
		return fmt.Sprintf("large model %q not permitted by model policy", info.ModelLarge)
	case !info.allowsModel(info.ModelMedium):
		return fmt.Sprintf("medium model %q not permitted by model policy", info.ModelMedium)
	case !info.allowsModel(info.ModelSmall):
		return fmt.Sprintf("small model %q not permitted by model policy", info.ModelSmall)
	}
	return ""
}
