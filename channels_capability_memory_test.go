...
func TestChannelsCapabilityPayloadDocumentsSeparatedTools(t *testing.T) {
	...
	for _, want := range []string{
		...
		"Use `channels_send` for a direct `[chat]` turn",
		"later outcome of work explicitly requested in that chat",
		"Internal Apteva chat replies are conversation-scoped",
		"If your current core thread is main or any non-conversation worker",
		"use core `send(id=\"<originating chat thread>\"",
		...
		"wait for that send to succeed",
		"never send the acknowledgement in parallel with action tools",
		"An earlier acknowledgement never replaces that final",
		"tool-work turn therefore has either one complete final message or two intentional messages",
		"acknowledgement then final",
		"never more",
		"prefer the acknowledgement before the handoff",
		"Never acknowledge both before and after the handoff",
		"The main receipt confirms delivery only, not completion",
		"After main replies, send exactly one final outcome",
		"operator disconnects before the work finishes",
		"Do not use ordinary chat for autonomous or scheduled checks",
		"unchanged or no-op results",
		"\"send a status update\" or \"update the status\" means `channels_set_status`",
		"update `channels_set_status` at most once",
		"call `pace` for the next due check and remain silent",
		"`next_at` is display metadata and does not schedule the next wake",
		...
		"Every direct `[chat]` turn requires at least one successful `channels_send`",
		"turn is incomplete until its user-visible answer is sent",
		"Thoughts and plain assistant output do not count",
		"ask a clarifying question",
		"report a read-only lookup",
		"After any non-channel tool result used for the request",
		"including a read-only lookup",
		"never call `pace` or `done` while a visible reply is still owed",
		"the next action must be the required `channels_send` outcome",
		"successful `channels_send` result is the delivery receipt",
		"An earlier acknowledgement does not replace this final outcome",
		"Never repeat that message",
		"Never leave the user-facing answer only in thoughts",
		"use `channels_send` for a requested outcome even if the operator disconnected",
		"does not justify unsolicited chat from autonomous work",
