...
func TestChannelsCapabilityPayloadDocumentsSeparatedTools(t *testing.T) {
	...
	for _, want := range []string{
		"`channels_send(channel, text, phase?, components?)`",
		...
		"The acknowledgement never replaces that final",
		"tool-work turn therefore has exactly two intentional messages",
		"acknowledgement then final",
		"never more",
		"send the required acknowledgement before the handoff",
		"never after it",
