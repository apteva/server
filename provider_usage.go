...
func (s *Server) handleProviderUsage(w http.ResponseWriter, r *http.Request) {
	...
	idPart := strings.TrimSuffix(path, "/usage")
	...
	fetcher := providerUsageFetcherFor(providerKey)
	if fetcher == nil {
		writeJSON(w, ProviderUsageSnapshot{Supported: false, ProviderID: providerID})
		return
	}
	...
	snapshot, err := s.fetchProviderUsage(r.Context(), userID, provider, providerKey, state, fetcher, r.URL.Query().Get("refresh") == "1")
	if err != nil {
...
func (s *Server) fetchProviderUsage(ctx context.Context, userID int64, provider *Provider, providerKey string, state map[string]any, fetcher providerUsageFetcher, force bool) (*ProviderUsageSnapshot, error) {
	...
	if providerKey == openAICodexAuthProvider && codexStateNeedsRefresh(state, 10*time.Minute) {
		refreshed, _, err := s.store.RefreshOpenAICodexProviderState(provider.ID, userID, s.secret, 10*time.Minute, false, "provider_usage")
		if err != nil {
			return providerUsageStaleOrError(provider.ID, staleEntry, hasStale, err)
		}
		state = refreshed
	}

	snapshot, err := fetcher.FetchUsage(ctx, state)
	if err != nil {
		return providerUsageStaleOrError(provider.ID, staleEntry, hasStale, err)
	}
	now := time.Now().UTC()
	...
	nextKey := fetcher.CacheKey(state)
	globalProviderUsageCache.put(nextKey, providerUsageCacheEntry{snapshot: *snapshot, fetched: now})
	return snapshot, nil
}
