...
// TestEval_InWorld_RealLLM_Seeded proves the seed step: the World is populated
...
// created them in the in-world storage and the agent read them back.
//
// Gated (OpenCode key + core binary + storage source):
//   go test -run TestEval_InWorld_RealLLM_Seeded -v -timeout 600s
func TestEval_InWorld_RealLLM_Seeded(t *testing.T) {
	...
	// Bind storage so the world installs it.
	...
	// Seed: two real files in the in-world storage, BEFORE the agent runs.
	seed := []SeedCall{
	...
	defer cancel()
	run, err := s.runEval(ctx, userID, agent, ev, RunOptions{UseWorld: true, SeedPlan: seed})
	if err != nil {
	...
	// wrote them to the in-world storage before the agent looked.
	...
	t.Logf("✓ world seeded (2 files) before the agent ran; agent read the seeded state via real storage; verdict=%s", run.Status)
}
