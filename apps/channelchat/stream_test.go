...
func TestFormatAgentChatEventIncludesReplyContract(t *testing.T) {
	...
	for _, want := range []string{
		"[chat]",
		"prefer to first send one short visible acknowledgement",
		"strong guidance, not a hard requirement",
		"wait for its successful delivery receipt before calling action tools",
		"send exactly one complete final result",
		"Thoughts and plain assistant output are not visible to the user and do not count as a reply",
		"through channels_send with channel=\"current\" or channel=\"apteva\"",
		"Never send the same answer twice after a successful delivery receipt",
		"DASHBOARD CHAT COMPLETION REQUIREMENT",
		"one optional acknowledgement before action tools",
		"an earlier acknowledgement does not satisfy it",
		"DURABLE HANDOFF",
		"normally acknowledge first",
		"never acknowledge both before and after the handoff",
		"Exactly one final outcome follows main's reply",
		"User message:",
		"What can you do?",
		"Dashboard context:",
		"- page: Overview",
