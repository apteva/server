...
func TestCallback_AppCallBatchUsesSingleTargetRequestAndRawResults(t *testing.T) {
	...
	targetHTTP := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Header.Get("X-Apteva-Subject-Type") != "user" || r.Header.Get("X-Apteva-Subject-ID") != "19" || r.Header.Get("X-Apteva-Project-ID") != "proj-1" {
			t.Errorf("principal headers=%v", r.Header)
		}
		principal, err := sdk.PrincipalFromRequest(r)
		if err != nil || principal == nil || principal.UserID != 19 || principal.ProjectID != "proj-1" {
			t.Errorf("trusted principal=%+v err=%v", principal, err)
		}
