...
func (s *Server) interfacePreparationStatus() map[string]string {
	...
	if err != nil {
		return map[string]string{"status": "not_installed", "message": "Conversations is not installed"}
	}
...
// Prepare AI-assisted onboarding only after that route is explicitly selected,
// without choosing an interface or
// completing onboarding. Retry uses the existing installation and preserves data.
func (s *Server) handlePrepareOnboarding(w http.ResponseWriter, r *http.Request) {
	...
	// Old clients called this before selecting a route. Keep those requests
	// harmless so a cached dashboard cannot provision apps during manual setup.
	switch body.Mode {
	case "", "manual":
	...
	case "ai":
	default:
		http.Error(w, "mode must be ai or manual", http.StatusBadRequest)
		return
	}
	if !s.prepareHelperConversations(w, userID) {
		return
	}
	writeJSON(w, map[string]string{"status": "ready"})
}
